...
import { Database, getDatabase, onValue, ref, get, query, orderByKey, startAt, endAt } from "firebase/database";
...
export class FirebaseService {
   ...
   */
  public subscribe<T>(path: string, callback: (payload: T | null) => void): void {
  ...
  public async fetchHistory<T>(path: string, startTs: number, endTs: number): Promise<T[]> {
    const dbRef = ref(this.database, path);
